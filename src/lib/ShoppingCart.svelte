...
<div transition:fade="{{duration:500}}" class="z-50 max-h-screen overflow-hidden flex justify-end w-full absolute inset-0 bg-black/50">
			...
			{#each cartItems as item, i (item.name)}
				<div animate:flip>
					<div class="w-full flex mb-2">
						<img class="bg-white flex-none w-20" src={item.src} alt="" />
					...
					<div class="flex w-full mb-4">
						<div class="flex w-full h-8 border border-white/40">
							<div class="px-2 h-full flex items-center text-white-300">
								<p class="uppercase font-medium text-white">{item.quantity}</p>
							</div>
							<button
								on:click={() => {
									removeOne(i);
